...
export function Auction() {
    ...
    const processingAuctions = useRef([])
    ...
    const sortAuctions = useCallback((auctions, criteria, order) => {
        if (criteria !== "action") {
        ...
        async function fetchAuctions() {
            ...
            if (result) {
                for (const auction of auctions) {
                    // Prevent the same auction from being processed multiple times.
                    if (processingAuctions.current.includes(auction._id)) {
                        continue
                    }
                    // Check if auction is expired.
                    if (auction.expirationTime > Date.now()) { continue }
                    processingAuctions.current.push(auction._id)
                    // Fetch buyer details.
                    const buyer = users.find(u => u._id === auction.highestBidderId)
                    // Process auction settlement.
                    if (buyer && !buyer.wonAuctions.includes(auction._id)) {
                    ...
                    processingAuctions.current = processingAuctions.current.filter(id => id !== auction._id)
                }
            }
    ...
    function calculateNextPrice(price) {
        return price < 10 ? price + 1 : Math.round(price * 1.1)
    }
    ...
    async function handleBid(auctionId, price) {
        // Fetch the latest auction data from the server.
        const auctionResult = await service.readAuctions()
        const auctionFromServer = auctionResult.auctions.find(a => a._id === auctionId)
        // Fetch the latest users data from the server.
        const usersResult = await service.readUsers()
        const serverUsers = usersResult.users
        // Update Redux users with fresh server data.
        dispatch(usersActions.setUsers(serverUsers))
        // Calculate the next price based on the current bid.
        const nextPrice = calculateNextPrice(auctionFromServer.price)
        // Find the previous bidder details from the server data.
        const previousBidder = serverUsers.find(u => u._id === auctionFromServer.highestBidderId)
        ...
        if (previousBidder) {
            ...
            // If the current user is the previous bidder, update their information.
            if (user._id === previousBidderToBeRepaid._id) {
                dispatch(userActions.setUser(previousBidderToBeRepaid))
                localUser.set(previousBidderToBeRepaid)
            }
        ...
        const highestBidderToBeUpdated = {
            ...user,
            wallet: user.wallet - Math.ceil(auctionFromServer.price),
        ...
        localUser.set(highestBidderToBeUpdated)
                ...
                <div className="auctionCard" key={a._id}>
                    <div className="cardInfo">
                        ...
                        <p className="auctionDuration">
                            ...
                            {a.expirationTime > Date.now() && formatTime(a.expirationTime - Date.now()).clock}
                        </p>
